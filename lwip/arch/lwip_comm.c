...
void lwip_comm_default_ip_set(__lwip_dev *lwipx)
{
    ...
    if(MAC_ADDR != 0xFFFFFFFF)
    {
        lwipx->mac[0] = MAC_ADDRESS(0);       /* 高三字节(IEEE称之为组织唯一ID,OUI)地址固定为:2.0.0 */
        lwipx->mac[1] = MAC_ADDRESS(1);
        lwipx->mac[2] = MAC_ADDRESS(2);
        lwipx->mac[3] = MAC_ADDRESS(3);       /* 低三字节用STM32的唯一ID 28.0.36 */   
...
void lwip_link_thread( void * argument )
{
    ...
    while(1)
    {
        ...
        if((regval & PHY_LINKED_STATUS) == 0)
        {
            ...
            {
                printf("The network cable has been unplugged!!!\r\n");
#if LWIP_DHCP                                           /* 如果使用DHCP的话 */
                g_lwip_dhcp_state = LWIP_DHCP_LINK_DOWN;

...
TForteInt32 CIEC_ANY_DATE::getTimeZoneOffset(){
  if(sm_nTimeZoneOffset == -1){
    ...
    //struct tm *ptm = forte_localtime(&t);
      struct tm_test *ptm = (struct tm_test *)forte_localtime(&t);
    if(ptm->tm_mday < 2) {
...
bool CIEC_ANY_DATE::setDateAndTime(struct tm &pa_rstTM, unsigned int pa_unMilliSec){
    ...
    struct tm_test *pa_rstTM_temp = (struct tm_test *)&pa_rstTM;
  //pa_rstTM.tm_isdst = -1;
    pa_rstTM_temp->tm_isdst = -1;

  time_t nTime;
  nTime = forte_mktime((struct tm_test*)&pa_rstTM);
...
struct tm *CIEC_ANY_DATE::getTimeStruct() const {
  ...
  pstRetVal = (struct tm *)forte_localtime(&nTime);
  return pstRetVal;
}

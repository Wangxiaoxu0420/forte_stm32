...
void ATK_MW1278D_UART_IRQHandler(void)
{
    ...
    if (__HAL_UART_GET_FLAG(&g_uart_handle, UART_FLAG_RXNE) != RESET)       /* UART接收中断 */
    {
        ...
        //StartTimeoutCheck();
		if(g_uart_rx_frame.sta.finsh == 0){
			if (g_uart_rx_frame.sta.len < (ATK_MW1278D_UART_RX_BUF_SIZE - 1))   /* 判断UART接收缓冲是否溢出
																				 * 留出一位给结束符'\0'
																				 */
			{
	//			if((tmp == 0x0A && g_uart_rx_frame.buf[g_uart_rx_frame.sta.len-1]==0x0D))
	//			{
...
//		if((g_uart_rx_frame.sta.len>2)&&(g_uart_rx_frame.buf[g_uart_rx_frame.sta.len-1] == 0x0A && g_uart_rx_frame.buf[g_uart_rx_frame.sta.len-2]==0x0D))
//		{
//			g_uart_rx_frame.sta.finsh = 1;
			//if(xEventGroupSetBitsFromISR(xGatewayEventGroup,LORA_REC_EVT,&xHigherPriorityTaskWoken)!= pdFAIL)

...
extern void HAL_UART6_MspInit(UART_HandleTypeDef *huart);
/**
 * @brief       UART底层初始化函数
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
		...
		else if (huart->Instance == ATK_MW1278D_UART_INTERFACE)                 /* 如果是ATK-MW1278D UART */
    {
        ...
        HAL_NVIC_SetPriority(ATK_MW1278D_UART_IRQn, 5, 0);                  /* 抢占优先级0，子优先级0 */
...
extern void HAL_UART6_RxCpltCallback(UART_HandleTypeDef *huart);
/**
 * @brief       Rx传输回调函数
 * @param       huart: UART句柄类型指针
 * @retval      无

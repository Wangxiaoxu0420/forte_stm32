...
//#include "log.h"
    ...
    #endif
    #if SRAMBANK >= 2
    static __align(64) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((section(".IRAM2"), aligned(1)));     /* 内部CCM内存池 */
    #endif
    #if SRAMBANK >= 3
    ...
    #endif
    #if SRAMBANK >= 2
    static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".IRAM2"), aligned(1)));  /* 内部CCM内存池MAP */
    #endif
    #if SRAMBANK >= 3
...
void *GWmalloc(uint32_t size)
{
    void * ptr;

    //uint16_t mem_use_rate = my_mem_perused(SRAMCCM);
	//printf("Send Mem USE RATE before malloc：%d\n",mem_use_rate);
	ptr =  mymalloc(SRAMIN,size);
    
    //检测内存泄漏，每次分配是否在逐次递减，
    //printf("\r\n malloc address is 0x%x \r\n", ptr);
    
    //size_t mem_use_rate = my_mem_perused(SRAMIN);
	//printf("malloc %d byte on %p,Mem USE RATE after malloc：%d,\n",size,ptr,mem_use_rate);
    return ptr;
}
void *GWrealloc(void *ptr, uint32_t size)
{

    //uint16_t mem_use_rate = my_mem_perused(SRAMCCM);
	//printf("Send Mem USE RATE before malloc：%d\n",mem_use_rate);
	ptr =  myrealloc(SRAMIN,ptr,size);
...
{
    //检测内存泄漏，是否及时释放
    //printf("\r\n free address is 0x%x \r\n", ptr);
    
    //uint16_t mem_use_rate;
	myfree(SRAMIN,ptr);
    //mem_use_rate = my_mem_perused(SRAMCCM);
	//printf("free mem at %p,Mem USE RATE after free:%d\n",ptr,mem_use_rate);
}
